...
const DashboardOverview = ({ stats, setActiveTab, loading, doctorData }) => {
  const [todayAppointments, setTodayAppointments] = useState([]);
  const [upcomingAppointments, setUpcomingAppointments] = useState([]);

  // ✅ FIXED: Use doctorData.isFullyVerified instead of individual props
  const isProfileApproved = doctorData?.isFullyVerified || false;

  console.log('📊 DashboardOverview received doctorData:', doctorData);
  console.log('🔓 Dashboard isProfileApproved:', isProfileApproved);

  useEffect(() => {
    if (isProfileApproved) {
      console.log('✅ Profile approved, fetching today\'s schedule');
      fetchTodaySchedule();
    } else {
      console.log('❌ Profile not approved, skipping schedule fetch');
    }
  }, [isProfileApproved]);
  ...
  const getProfileStatusMessage = () => {
    ...
    console.log('🎭 Getting status message for:', { status, profileComplete, documentsUploaded, hasAllDocuments });
    
    if (status === 'APPROVED' && profileComplete && documentsUploaded && hasAllDocuments) {
      return {
        title: 'Profile Fully Verified!',
        message: 'Your profile has been approved and all features are now available.',
        icon: CheckCircle,
        color: 'green'
      };
    ...
    } else if (status === 'APPROVED' && (!documentsUploaded || !hasAllDocuments)) {
      return {
        title: 'Documents Required',
        message: 'Your profile is approved but please upload all required documents to unlock all features.',
        icon: AlertCircle,
        color: 'orange'
      };
    } else {
  ...
  // ✅ FIXED: Show profile completion message if not fully verified
  if (!isProfileApproved) {
      ...
      <div className="p-6 min-h-screen bg-gray-50">
        <div className={`rounded-xl border p-8 text-center ${colorClasses[statusInfo.color]} max-w-4xl mx-auto mt-20`}>
          ...
          <div className="space-y-4">
            {(!doctorData.profileComplete || doctorData.status === 'REJECTED') && (
              <button
              ...
              >
                ...
                {!doctorData.profileComplete ? 'Complete Profile Setup' : 'Review & Resubmit Profile'}
              </button>
            )}
            
            {doctorData.status === 'APPROVED' && (!doctorData.documentsUploaded || !doctorData.hasAllDocuments) && (
              <button
                onClick={() => setActiveTab('profile')}
                className="bg-orange-600 text-white px-8 py-3 rounded-lg hover:bg-orange-700 transition-colors font-semibold flex items-center mx-auto"
              >
            ...
            {doctorData.profileComplete && doctorData.status === 'PENDING' && (
              <div className="bg-white rounded-lg p-6 max-w-md mx-auto">
                ...
                <ul className="text-sm space-y-3 text-left">
                  <li className="flex items-center">
                    <CheckCircle size={16} className={`mr-3 flex-shrink-0 ${doctorData.profileComplete ? 'text-green-600' : 'text-gray-400'}`} />
                    <span className={doctorData.profileComplete ? 'text-green-800' : 'text-gray-600'}>
                      Profile information submitted
                    </span>
                  </li>
                  <li className="flex items-center">
                    <CheckCircle size={16} className={`mr-3 flex-shrink-0 ${(doctorData.documentsUploaded && doctorData.hasAllDocuments) ? 'text-green-600' : 'text-gray-400'}`} />
                    <span className={(doctorData.documentsUploaded && doctorData.hasAllDocuments) ? 'text-green-800' : 'text-gray-600'}>
                      Documents uploaded
                    </span>
                  </li>
                  <li className="flex items-center">
                    <Clock size={16} className="text-yellow-600 mr-3 flex-shrink-0" />
                    <span className="text-yellow-800">Admin review in progress</span>
                  </li>
                </ul>
  ...
  // ✅ Dashboard for fully verified users only
  const statCards = [
    { 
        ...
        <div className="flex items-center">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-900">
              Welcome back, Dr. {JSON.parse(localStorage.getItem('user')).firstName}!
            </h2>
